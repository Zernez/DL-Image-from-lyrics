...
class DiscriminatorDecider2(nn.Module):
    def __init__(self, config):
        ...
        self.conv3= nn.Conv2d(ndf * 2, ndf * 2, kernel_size=4, stride=2, padding=1, bias=False) 
        self.norm2= nn.BatchNorm2d(ndf * 2)
        self.leaky3= nn.LeakyReLU(0.1, inplace=False)
        self.conv4= nn.Conv2d(ndf * 2, ndf * 4, kernel_size=4, stride=2, padding=1, bias=False)
        self.norm3= nn.BatchNorm2d(ndf * 4)
        self.leaky4= nn.LeakyReLU(0.1, inplace=False)
        self.conv5= nn.Conv2d(ndf * 4, ndf * 4, kernel_size=4, stride=2, padding=1, bias=False)     
        self.norm4= nn.BatchNorm2d(ndf * 4)
        self.relu= nn.ReLU(inplace=False)
        self.conv6= nn.Conv2d(ndf * 4, ndf * 8, kernel_size=4, stride=2, padding=1, bias=False)     
        self.norm5= nn.BatchNorm2d(ndf * 8)
        self.conv7= nn.Conv2d(ndf * 8, self.out_channels, kernel_size=4, stride=2, padding=1, bias=False)
