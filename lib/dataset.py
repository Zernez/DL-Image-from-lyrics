...
#            word_vectors_tensor = torch.Tensor(word_vectors)
            word_vectors_tensor = torch.Tensor(np.array(word_vectors))
...
#            similar_vector1 = torch.Tensor(self._get_similar_wv_by_vector(word_vector)).unsqueeze(0)
            sim_vector= self._get_similar_wv_by_vector(word_vector)
            sim_vector.setflags(write=1)
            similar_vector = torch.from_numpy(sim_vector).unsqueeze(0)
...
#            fake_word_vectors = torch.Tensor(self._generate_dissimilar_wvs(word_vectors))
            fake_word_vectors = torch.Tensor(np.array(self._generate_dissimilar_wvs(word_vectors)))
